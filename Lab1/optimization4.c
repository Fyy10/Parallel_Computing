...
int main(int argc, char *argv[]) {
    ...
    // P0
    if (!id) {
        for (base_idx = 0; base_idx < size; base_idx += cache_size) {
            index = 0;
            prime = 3;
            do {
                low_value_cache = low_value + base_idx;
                if (prime * prime > (2 * (low_value_cache) + 1))
                    first = (prime * prime - 1) / 2 - low_value_cache;
                else {
                    // map low_value to unmarked numbers
                    int low_value_prime = 2 * low_value_cache + 1;
                    ...
                    if (!(low_value_prime % prime)) first = 0;
                        ...
                        if (k & 1)
                            first = (k * prime - 1) / 2 - low_value_cache;
                            // k * prime is even, then k * prime + prime is the first number to mark
                        else
                            first = ((k + 1) * prime - 1) / 2 - low_value_cache;
                    }
                }
                for (i = first + base_idx; i < first + base_idx + cache_size && i < size; i += prime) marked[i] = 1;
                while (marked[++index]);
                prime = 2 * (index + 1) + 1;
            } while (prime * prime <= n);
        }
