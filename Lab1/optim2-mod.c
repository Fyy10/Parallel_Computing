...
int main(int argc, char *argv[]) {
    ...
    // compute local prime list as well as P0's marked array
    do {
        // low_value for local_prime_list is 1
        // prime * prime > 2 * 1 + 1
        first = (prime * prime - 1) / 2 - 1;
        // P0
        if (!id) for (i = first; i < size; i += prime) marked[i] = 1;
        // others
        else for (i = first; i < prime_list_size; i += prime) local_prime_list[i] = 1;
        // P0
        if (!id) while (marked[++index]);
        // others
        else while (local_prime_list[++index]);
        prime = 2 * (index + 1) + 1;
    } while (prime * prime <= n);
